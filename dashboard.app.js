...
document.getElementById("descBody").textContent="Loading…";
...
document.getElementById("descMeta").textContent=[company ? ("Company: "+company) : "", source ? ("Source: "+source) : "", fetchedAt ? ("Fetched: "+fetchedAt) : ""].filter(Boolean).join(" · ");
...
async function generateTailoredCv(jobId, force){
  try{
    setText("tailorStatus", "Generating…");
    ...
    setText("tailorMeta", `Template: ${template} · Strength: ${strength}`);
...
function cvDocToPreviewHtml(doc, lang){
  ...
  const expHtml = exp.length ? exp.slice(0, 12).map(e=>{
    ...
    return [
      `<div class="cvItem">`,
      `<div class="cvItemTitle">${escapeHtml(joinNonEmpty([t, company], " — ") || (t||company||""))}</div>`,
  ...
  const eduHtml = edu.length ? edu.slice(0, 10).map(e=>{
    ...
    const school = joinNonEmpty([e?.school, e?.location], " · ");
  ...
  const coursesInner = courses.length ? `<div class="cvPara">${escapeHtml(courses.join(" · "))}</div>` : "";
  const interestsInner = interests.length ? `<div class="cvPara">${escapeHtml(interests.join(" · "))}</div>` : "";
  const langsInner = langs.length ? `<div class="cvPara">${escapeHtml(langs.join(" · "))}</div>` : "";
...
function updateTailorButtonsState(isGenerating){
  ...
  if(gen){
    if(isGenerating) gen.textContent = "Generating…";
...
function cvDocToPlainText(doc, lang){
  ...
  if(exp.length){
    ...
    for(const e of exp){
      const head = joinNonEmpty([e?.title, e?.company], " — ");
      const meta = joinNonEmpty([fmtPeriod(e?.start, e?.end), e?.location], " | ");
  ...
  if(courses.length){ addTitle(L.courses); lines.push(courses.join(" · ")); }

  // Interests
  const interests=asStringArr(doc?.interests, 80);
  if(interests.length){ addTitle(L.interests); lines.push(interests.join(" · ")); }

  // Languages
  const langs=asStringArr(doc?.languages, 40);
  if(langs.length){ addTitle(L.languages); lines.push(langs.join(" · ")); }
  ...
  return `
    <div class="${wrapClass}">
      <div class="item">
        ...
        <div class="tagRow">
          <span class="tag ok">Fresh ≤2d: <b>${freshCount}</b></span>
          <span class="tag ${prioCount? "ok":"warn"}">Prioritized: <b>${prioCount}</b></span>
          <span class="tag ${staleCount? "warn":"ok"}">Stale ≥10d: <b>${staleCount}</b></span>
        </div>
        <div class="hr"></div>
...
if(metaTxt === "—" && ev.meta && typeof ev.meta === "object"){
if(t === "sent"){
...
const link=job.apply_url?String(job.apply_url):"";
const titleCell=link ? ('<a href="'+escapeHtml(link)+'" target="_blank" rel="noopener">'+escapeHtml(title)+'</a>') : escapeHtml(title);
const actionBtn=(st==="skipped" && job.id) ? "<button class='btn small ghost' type='button' data-unskip='"+escapeHtml(job.id)+"'>Undo</button>" : "—";
...
function renderActivitySnapshot(items){
  ...
  const meta = '<div class="small" style="margin:8px 0 10px;color:rgba(17,19,24,.72)">'
    ...
    + (prioritized ? (' · Prioritized: <b>'+escapeHtml(String(prioritized))+'</b>') : '')
    + '</div>';
...
setText("aiInsightsHint", created ? ("Last updated: "+created) : "Last updated: —");
...
if(qaTailor){
  qaTailor.addEventListener("click",()=>{
    try{
      ...
      if(!jobs.length){
        setText("qaStatus","No jobs in your queue yet. Click “Search jobs (fetch)” first.");
        return;
      }
